...
text-bridging binary defaults on ``Storage``, and the coherence of the
``storage_module`` manifest rename + the ``resource_manager`` facet rename.
...
# -- Manifest field rename (hard break) -----------------------------------------
...
def test_manifest_template_module_is_gone():
...
def test_app_storage_facet_exposes_resource_manager():
    ...
    assert not hasattr(AppStorage, "template_manager")
