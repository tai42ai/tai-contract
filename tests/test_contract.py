...
# (webhook_verifiers + channels), so the distinct-name union (53) is three
# fewer than the pair count (56).
EXPECTED_FACADE = {
    # tools (11)
    "tool",
    "toolkit",
...
def test_facade_partition_against_frozen_surface():
    ...
    # 56 (sub-protocol, member) pairs over 53 distinct names — ``store`` is
    # exposed by both AppVersioning and AppPresets, and ``register``/``get``
    # by both AppWebhookVerifiers and AppChannels.
    assert len(union) == 53, f"union={len(union)}"
    assert total == 56 == len(union) + 3, f"partition broken: sum={total} union={len(union)}"
...
def test_backend_surface_is_task_runtime_only():
    ...
    # abstract member. The whole fleet control-plane surface is gone — a
    # concrete backend implementing only ``launch`` instantiates cleanly, and
    ...
    for name in removed:
        assert not hasattr(Backend, name), f"{name} must be gone from the Backend contract"
...
def test_connector_token_store_has_no_tenant():
    ...
    for name in ("get", "put", "delete"):
        ...
        assert "tenant_id" not in params, f"{name} still takes tenant_id"
    # the store exposes a plain `list` (no per-tenant variant)
    assert hasattr(ConnectorTokenStore, "list")
    assert not hasattr(ConnectorTokenStore, "list_for_tenant")
