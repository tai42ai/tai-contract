...
installed. Recreate the venv without the `dev` extra, run the rebuild, then put
the dev tooling back:

```bash
uv venv --python 3.13
uv pip install --no-sources --editable .
uv run --no-sync python -c "import importlib,pkgutil,pydantic,tai42_contract; \
