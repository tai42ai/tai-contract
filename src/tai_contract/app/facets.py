"""Per-feature sub-protocols composed by :class:`~tai_contract.app.TaiApp`.
...
from tai_contract.sub_mcp import SubMcpAppRouter
...
class AppWebhookVerifiers(Protocol):
    def register(self, name: str, verifier: WebhookVerifier) -> None:
        ...
        A provider plugin calls this through the ``tai_app`` handle when its
...
class AppChannels(Protocol):
    def register(self, name: str, channel: Channel) -> None:
        ...
        A channel plugin calls this through the ``tai_app`` handle when its
...
class AppConnectors(Protocol):
    def register_connector(self, descriptor: ProviderDescriptor) -> None:
        ...
        provider plugin calls this through the ``tai_app`` handle when the manifest
...
class AppPresets(Protocol):
    ...
    ``store`` is the :class:`~tai_contract.presets.PresetStore`-typed view over
