...
(``namespace/name``), the pip distribution that backs it, the tai-contract
...
The YAML I/O helpers live above the contract (``tai_kit.plugins``) — the
contract itself has no YAML dependency.

``KIND_MANIFEST_BINDINGS`` is the single source for how each provided item
kind wires into a :class:`~tai_contract.manifest.Manifest`: which manifest
...
class PluginSpec(BaseModel):
    ...
    ingest validation both pin that); ``contract`` is the tai-contract
